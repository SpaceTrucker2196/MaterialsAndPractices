...
struct CurrentGrowsContentView: View {
    ...
    @FetchRequest(
        sortDescriptors: [NSSortDescriptor(keyPath: \Grow.timestamp, ascending: true)],
        animation: .default)
    private var items: FetchedResults<Grow>

    var body: some View {
        List {
            ForEach(items) { item in
                Text("Grow Started: \(item.timestamp!, formatter: itemFormatter)")
            }
