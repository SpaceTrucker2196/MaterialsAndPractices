...
struct HarvestSafetyChecklistItem: Identifiable {
    let id = UUID()
    let text: String
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                ForEach($checklistData.sections.indices, id: \.self) { sectionIndex in
                    Section(header: Text(checklistData.sections[sectionIndex].title)) {
                        ForEach($checklistData.sections[sectionIndex].items) { $item in
                            HarvestChecklistItemView(item: $item)
                        }
                    }
                }
...
struct HarvestChecklistItemView: View {
    @Binding var item: HarvestSafetyChecklistItem
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Button(action: {
                    ...
                    if item.isCompleted { item.isNotApplicable = false }
                }) {
                ...
                Button(action: {
                    item.isNotApplicable.toggle()
                    if item.isNotApplicable { item.isCompleted = false }
                }) {
                    Text("N/A")
                        .font(.caption)
