        ...
        NavigationView {
            ...
            .sheet(isPresented: $showingAmendmentDetail) {
                if let amendment = selectedAmendment {
                    AmendmentDetailView(amendment: amendment, isPresented: $showingAmendmentDetail)
                }
            }
        }
    }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        ...
                        HStack(spacing: 4) {
                            ...
                            if let currentInventory = amendment.currentInventoryAmount {
        ...
        VStack(alignment: .leading, spacing: AppTheme.Spacing.medium) {
            ...
            VStack(spacing: AppTheme.Spacing.small) {
                if let currentInventory = amendment.currentInventoryAmount {
                    InfoRow(
                        label: "Current Stock",
                        value: "\(currentInventory, specifier: "%.1f") units"
                    )
                    ...
                    HStack {
                        ...
                        Text(currentInventory < 10 ? "Low Stock" : "In Stock")
                            .font(AppTheme.Typography.bodyMedium)
                            .foregroundColor(currentInventory < 10 ? AppTheme.Colors.warning : AppTheme.Colors.success)
                    }
                } else {
...
struct CreateAmendmentView: View {
    ...
    private func createAmendment() {
        ...
        newAmendment.supplier = selectedSupplier
...
struct EditAmendmentView: View {
    ...
    private func loadAmendmentData() {
        ...
        selectedSupplier = amendment.supplier
        
        if let inventory = amendment.currentInventoryAmount {
    ...
    private func saveChanges() {
        ...
        amendment.supplier = selectedSupplier
